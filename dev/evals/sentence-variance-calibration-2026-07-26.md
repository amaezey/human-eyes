...
Record for DR-79A: #52 `sentence-length-variance` moves from a standard
deviation threshold of 4 to 9, and no average-sentence-length check was built.

## The check was inert

#52 fails a document when the standard deviation of its sentence word counts
...
#52's eligibility rule in 2026-07-17 and did not touch it. The one source that
...
At 9 this is the widest separation in the catalogue. For reference #65 runs
70%/24%, #10 66%/24%, #68 57%/29%, #66 52%/27%, #69 50%/18%, and the #25
...
averages 2,172 words per document against the generated corpus's 1,051. #52 is a
...
## Average sentence length: DR-79B, folded into #25
...
Mae chose 15.0, in prose of 300 words or more, folded into #25 rather than
...
- **Within #25**, it adds 9 generated and 3 human documents, taking #25 from
  54% to 74% on generated prose and 53% to 58% on human prose.
- **Against the union of the retuned #52 and #25**, it adds 1 generated
  document and 0 human ones, because mean and standard deviation correlate at
  r=0.71 and the #52 retune already absorbs most of the territory.

The second figure is the one that matters for deciding whether the project
gained coverage; the first is the one that matters for reading #25's rates.
...
Of the six static features Lu et al. name, type-token ratio is already #53. Noun
...
#65 already counts one slice of noun-heaviness through nominalisation suffixes.
