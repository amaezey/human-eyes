...
def _relative_report_path(readme_path: Path, structured: dict) -> str:
    return "dev/skill-workspace/latest-performance-report.md"


def write_workspace_report_archive(report_md: str, report_data: dict) -> None:
    """Mirror the iteration's perf report to two stable, tracked paths.

    - `latest-performance-report.md` is the README's permanent link target.
    - `reports/YYYY-MM-DD-iteration-N.md` archives each run as a single file
      so historical perf is browseable without expanding gitignored iter dirs.
    """
    iteration = report_data.get("iteration", "iteration-?")
    timestamp = report_data.get("timestamp") or ""
    date_slug = timestamp[:10] if len(timestamp) >= 10 else "unknown-date"

    latest = WORKSPACE / "latest-performance-report.md"
    latest.write_text(report_md, encoding="utf-8")

    reports_dir = WORKSPACE / "reports"
    reports_dir.mkdir(parents=True, exist_ok=True)
    archived = reports_dir / f"{date_slug}-{iteration}.md"
    archived.write_text(report_md, encoding="utf-8")
