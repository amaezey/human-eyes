...
- The strongest source-to-project overlap is its exact promotional phrase set. DR-133 completes C03-C05 coverage across #1, #4, #7, #8, and #22.
...
- #1 `no-significance-inflation`, #4 `no-promotional-language`, #7 `no-ai-vocabulary-clustering`, and #8 `no-copula-avoidance` recognise the complete C03-C04 exact phrase family.
- #22 `no-filler-phrases` recognises both forms of `it is/it's important to note` and `no discussion would be complete without`; #7 recognises `defining feature` and `powerful tools` as clustering candidates.
...
- #39 `no-placeholder-residue` covers constrained bracket, brace, and angle forms plus possessive labels like `[Subject's Name]` (added in d986dd5); the free-text `[URL of source]` fixture remains outside deterministic coverage.
...
- C12 and C13 approved 2026-07-17 via DR-114 components 1-2: commit d986dd5 added `citeturn` tokens, `contentReference[oaicite:...]` tokens, `utm_source=chatgpt.com` trackers, and possessive bracket labels such as `[Subject's Name]` to #39 `no-placeholder-residue`. DR-114 components 3 (quoted-example over-fire fix) and 4 (private-person gap-filling filler phrases) were declined, so #39 remains quote-unaware; C12's “unambiguous evidence” authorship claim stays non-promoted.
...
- C03-C05 approved 2026-07-19 via DR-133: #1 now recognises `underscores its importance` and `enduring legacy`; #4 recognises `rich cultural heritage`; #22 recognises contracted `it's important to note` and `no discussion would be complete without`; and `defining feature` plus `powerful tools` are #7 clustering candidates. Existing #1/#8 coverage already handled `stands as a testament`, `plays a vital role`, and `breathtaking landscapes`.
- C06 approved 2026-07-19 via DR-134 option B: `moreover`, `furthermore`, and `on the other hand` joined #50 as exact paragraph openers. Mae rejected connector-density logic.
...
| C03: Rohrer says the discussed linguistic patterns emerge from how models construct responses and often reflect training methodologies and optimization objectives; he says LLMs overemphasize significance with `stands as a testament`, `plays a vital role`, and `underscores its importance`, attributing this instance to promotional training content. | Broad author mechanism interpretation plus three examples; no model architecture, training or optimization evidence, sample, counts, model/date scope, human comparison, or citation. | fully covered for the exact phrases: #1 recognises all three and #8 additionally recognises `stands as`; DR-133 added the previously missed `underscores its importance` frame. | Candidate coverage is distributed across the matching surface mechanisms. | **adopt:** retain all three exact phrases as programmatic #1/#8 candidates. | approved | implemented |
| C04: Promotional language is especially common for culture, locations, and history; examples are `rich cultural heritage`, `breathtaking landscapes`, and `enduring legacy`; humans allegedly vary more and avoid superlatives. | Author register comparison with three examples and an explicit human generalization; no corpus, frequency, coding, genre control, or model/date boundary. | fully covered for the exact phrases: #4 recognises `rich cultural heritage` and `breathtaking landscapes`; #1 recognises `enduring legacy`. | The phrase family spans promotional and significance mechanisms rather than one checker. | **adopt:** route the exact phrases through #4 and #1. | approved | implemented |
| C05: LLMs inject editorial voice through `it's important to note`, `no discussion would be complete without`, `defining feature`, and `powerful tools`; Rohrer attributes this to mixed training sources and says humans separate fact from interpretation more clearly. | Author observation, mechanism, and human comparison with no measured examples, attribution study, coding, or counterexamples. | fully covered for the exact phrases: #22 recognises both editorial frames, including straight and smart apostrophes; `defining feature` and `powerful tools` contribute to #7 clustering. | The exact phrases require two existing mechanisms because the adjective-noun forms are vocabulary candidates rather than filler frames. | **adopt:** route the editorial frames through #22 and the adjective-noun phrases through #7 clustering. | approved | implemented |
| C06: `moreover`, `furthermore`, and `on the other hand` become cues when used with mechanical regularity, although humans also use them. | Author frequency/regularity claim with a human counterexample; no count, density, span, sample, model/date, or threshold. | Covered for the three exact paragraph-opening forms by #50. | No density or distribution mechanism exists; Mae rejected adding one in DR-134. | Keep the exact #50 openers; add no connector-density mechanism. | approved | implemented |
...
- C03: **adopt** the three exact significance phrases under existing #1/#8 coverage.
- C04: **adopt** `rich cultural heritage`, `breathtaking landscapes`, and `enduring legacy` under #4/#1.
- C05: **adopt** the two editorial frames under #22 and the two adjective-noun phrases as #7 clustering candidates.
- C06: Keep the three exact paragraph openers in #50; add no connector-density or distribution measure.
...
- C03: passed - DR-133A assertions confirm #1 recognises `underscores its importance`; existing #1/#8 coverage handles the other two exact phrases.
- C04: passed - DR-133A assertions confirm #4 recognises `rich cultural heritage` and #1 recognises `enduring legacy`; `breathtaking landscapes` was already covered.
- C05: passed - DR-133A assertions confirm both #22 editorial frames, both apostrophe variants, both #7 candidates, and their clustering behaviour.
- C06: passed - DR-134B asserts that all three exact paragraph openers fail #50; no density mechanism was added.
...
- C12: passed - commit d986dd5 (DR-114 components 1-2) added the possessive bracket-label pattern covering `[Subject's Name]` to #39 `no-placeholder-residue`; `python3 dev/evals/tests/test_grade.py` passes the DR-114 assertions, including the editorial-bracket control, on 2026-07-17. Free-text `[URL of source]` remains outside deterministic coverage, and the “unambiguous” authorship claim was not adopted.
- C13: passed - commit d986dd5 (DR-114 components 1-2) added `citeturn`, `contentReference[oaicite:...]`, and `utm_source=chatgpt.com` patterns to #39 `no-placeholder-residue`; `python3 dev/evals/tests/test_grade.py` passes the DR-114 assertions on 2026-07-17.
