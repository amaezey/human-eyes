...
- C03, C07 approved 2026-07-26 via DR-131: Mae queued this work for later rather than ruling on its shape now. No checker, registry, or test change has been made and implementation has not started.
...
- C09 approved 2026-07-18 via DR-125: B1 now recognises `firstly`; `additionally` and the existing conclusion-scoped `overall` check were already covered.
...
- C07: Test-adapt the public matched pool by teacher grade and prompt before considering H10 guidance, contingent on reuse rights.
...
- C09: passed - DR-125 adds exact B1 recognition for `firstly`; `additionally` and the conclusion-scoped `overall` check remain covered.
