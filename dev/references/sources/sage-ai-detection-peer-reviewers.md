...
- Citation retired 2026-07-27 via DR-168: the root README credited SAGE for even jargon distribution, which this card's C09 states it does not cover (C09 concerns incorrect technical-term use, not terminology density). The pattern itself was then retired rather than re-sourced, because measurement on both corpora showed the level checks B5, B13 and B14 already catch 42 of the 46 generated documents and the evenness measure adds no coverage on the four they miss. The measurement is recorded in `dev/evals/jargon-evenness-measurement-2026-07-27.md`. C09 keeps its own `pending` status; nothing on this card was approved or rejected by that closure.
