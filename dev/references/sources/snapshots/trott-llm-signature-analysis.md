...
- **Canonical URL:** https://seantrott.substack.com/p/identifying-signatures-of-llm-generated
- **Alternate access URLs:**
  - https://seantrott.substack.com/api/v1/posts/identifying-signatures-of-llm-generated
  - https://substack-post-media.s3.amazonaws.com/public/images/6bfc7e2b-0907-4b91-8755-8dc12fbf493b_14000x5500.png
  - https://substack-post-media.s3.amazonaws.com/public/images/f70f42f1-f35f-48c4-b7aa-bef8db1f8825_14000x5500.png
- **Author / owner:** Sean Trott
- **Publisher:** The Counterfactual on Substack
- **Published:** 2025-04-18T20:41:44.443Z
- **Retrieved:** 2026-07-17
- **Stable identifier:** Substack post ID 161487215
- **Version / revision:** API `updated_at` 2025-04-18T20:42:54.109Z
- **Extraction method:** `curl -L` of the first-party Substack post API; Python 3 `html2text` conversion of the complete `body_html`; direct rendered-page HTML checked with BeautifulSoup; both original-resolution source figures downloaded from first-party Substack S3 URLs and visually inspected
- **Full-text status:** complete
- **Access and transformation notes:** The public API reports `audience: everyone`, `wordcount: 3156`, and a complete `body_html`. Navigation, subscription modules, comments, reactions, and unrelated page chrome were excluded. HTML was converted to Markdown, preserving the four article headings, three block quotations, two list items, two figure links and captions, all inline links, and all 12 numbered notes. The converter normalised whitespace and emits note markers as plain numbers; the exact API response is preserved below as an attachment. The two source figures are preserved byte-for-byte from their original S3 PNG URLs; their visible titles, axes, legends, distributions, and layer traces were checked, but no plotted value was reverse-engineered beyond the author's prose and captions.
...
### Synthetic text detection is a big field
...
Working on a similar intuition as the creators of DetectGPT, [another set of authors](<https://proceedings.neurips.cc/paper_files/paper/2023/file/7baa48bc166aa2013d78cbdc15010530-Paper-Conference.pdf>) argued that one might expect LLM-generated text to have a lower intrinsic dimensionality than human-generated text. That is, LLM-generated texts should be describable in fewer dimensions than human-generated texts.4 Using a particular measure of intrinsic dimensionality called _[persistent homology dimension](<https://en.wikipedia.org/wiki/Persistent_homology>)_ , the authors found that indeed, English texts produced by various LLMs had lower intrinsic dimensionality than texts on analogous topics produced by humans. Another interesting result was that their approach was less likely than other approaches to _misclassify_ text produced by non-native English speakers as synthetic, i.e., it had a lower false positive rate.5
...
As a starting point, I selected the dataset from [this 2023 paper](<https://pubmed.ncbi.nlm.nih.gov/37903836/>) analyzing argumentative essays written by (mostly) high school students on a fixed number of topics (e.g., “Should students be taught to cooperate or to compete?”).6 Using those same topics, the authors of that paper prompted ChatGPT-3 and ChatGPT-47 with the following instructions:

> Write an essay with about 200 words on “[topic]”

The authors carried out a number of analyses on these essays, including a human annotation study. For instance, they found that a separate pool of humans tended to rate the essays generated by the LLMs (particularly ChatGPT-4) as more “logical” and “complex” than those written by the students.8
...
[![](https://substackcdn.com/image/fetch/$s_!2OQQ!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Ff70f42f1-f35f-48c4-b7aa-bef8db1f8825_14000x5500.png)](<https://substackcdn.com/image/fetch/$s_!2OQQ!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2Ff70f42f1-f35f-48c4-b7aa-bef8db1f8825_14000x5500.png>)Left: distribution of I.D. scores (using the Two-NN algorithm) for each passage type. Scores were z-scored within layer, within model, to account for the effect of anisotropy. Higher scores indicate higher I.D., i.e., more dimensions were needed to represent the passage. Right: average I.D. across passage types for each layer in the model (also z-scored).
...
This is what I’ve previously called [the “moving target” problem](<https://seantrott.substack.com/p/llm-ology-and-the-moving-target-problem>).
...
- **Beginning checked:** The rendered public page and API body begin with the subscriber preface, horizontal divider, trust and construct-validity framing, false-positive warning, and the lead-in to the literature overview.
- **Middle checked:** The rendered page and API body contain the `My approach: the intuition` and `The results!` sections, the 2023 argumentative-essay dataset, the exact approximately-200-word prompt, four Pythia sizes, regression description, predictability results, and both preserved figures with captions.
- **End checked:** The rendered page and API body end with `The takeaway(s)`, the narrow two-model/student-essay conclusion, the warning against automated categorisation, and numbered notes 1 through 12; note 12 ends with the proposed but untested “team” of Pythias.
- **Structure checked:** First-party API `body_html` contains 55 paragraph nodes, four article `h3` headings, three block quotations, one two-item list, two figures, 50 links, and 12 numbered notes. The rendered canonical page contains one article and the same four article headings and two figures. The API metadata reports 3,156 words. Both 14,000 × 5,500 RGBA PNG figures were opened and compared with their captions and surrounding prose.
- **Known omissions:** None from the public article body. Dynamic comments, reactions, navigation, subscription prompts, and other page chrome are not source content and were not included.

## Preserved attachments

| Path | Role in the source | SHA-256 | Preservation / extraction notes |
|---|---|---|---|
| `snapshots/attachments/trott-llm-signature-analysis-substack-post-161487215.json` | Exact first-party post metadata and complete `body_html` | `ef41b2e56dba017b38d4ee2f70758cc58f99f2d257b6df0be0473414fd014604` | Direct Substack API response fetched with `curl -L`; used to reproduce the Markdown body and verify post ID, timestamps, audience, byline, word count, and structure. |
| `snapshots/attachments/trott-llm-signature-analysis-figure-1.png` | Predictability distributions: mean and standard deviation of surprisal | `ff7dd8e14fe676bb0e76907ec67c0a2e3bf9230fe3a3652145e9365d71b8a61a` | Original 14,000 × 5,500 RGBA PNG downloaded from the source S3 URL; title, four Pythia panels per measure, axes, ChatGPT-3/ChatGPT-4/student legend, and distribution ordering visually checked. |
| `snapshots/attachments/trott-llm-signature-analysis-figure-2.png` | Intrinsic-dimensionality distributions and layer traces | `135d352af9df18efac9434286d7e8bc0a39fc1806acdb8c559b16575b7d30287` | Original 14,000 × 5,500 RGBA PNG downloaded from the source S3 URL; title, Two-NN z-score axis, four Pythia panels, layer axes, condition legend, and traces visually checked. |
