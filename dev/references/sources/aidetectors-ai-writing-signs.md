...
- C06 approved 2026-07-19 via DR-134 option B: the three exact paragraph-opening transitions joined #50. Mae rejected a connector-density measure, so no density or distribution mechanism was added.
- C09 approved 2026-07-19 via DR-15 option A: the source's `Studies show` and `Research has found` forms are both covered by #5.
...
| C06: Paragraphs that repeatedly begin with formal transitions such as "Furthermore," "Moreover," and "Additionally" show transition-word overload; the page claims AI uses them at two to three times the human rate. | Direct phrase list and unsupported comparative multiplier; no corpus, unit of analysis, baseline, genre, or model/date details. | **covered for the exact forms:** #50 `no-formulaic-openers` recognises all three listed transitions at paragraph starts; #22 and #50 already recognise paragraph-initial `It is worth noting`. | No paragraph-initial denominator or connector-density mechanism exists; Mae rejected adding one in DR-134. | Keep the exact paragraph-opening forms in #50. Do not add connector-density or distribution logic. | approved | implemented |
...
- C06: Keep the three exact transition openers in #50 alongside the existing `It is worth noting` coverage; add no connector-density mechanism.
...
- C09: Keep both exact attribution forms in #5 and retain source verification separately.
...
- C06: passed - DR-134B asserts that `Furthermore`, `Moreover`, and `Additionally` paragraph openers fail #50; no connector-density mechanism was added.
...
- C09: passed - DR-133A covers `Studies show`; existing #5 coverage recognises `Research has found`.
