...
- The Yakura et al. question is answered: it was ingested directly on 2026-07-26 as `yakura-llm-influence-spoken-communication.md`. Should the linked Duke workplace-stigma study receive its own direct source review before any project evidence mapping is strengthened?
...
- The legacy card contained prose associations with H3, H9, C7, false-positive harm, and possible preserve guidance, but no claim-keyed user decisions or implementation statuses. The current evidence refresh reopens those associations as C01-C12 with `pending` / `not started`; nothing is carried forward as approved or implemented.
- C05 decided 2026-07-17: C7 remains fail-on-any (deliberate stance). The proposed register-matched C7 evaluation is closed with no product change. Recorded on C05 because it is the row that challenges C7's any-occurrence behaviour; C10 (transient-tell drift) was named in the decision batch but does not concern C7 thresholds and stays pending.
...
| C02: Clarke reports people purging em dashes, `delve`, `nestled`, `boast`, `meticulous`, and the `not just X, Y` construction despite claiming human authorship. | Journalistic synthesis with linked public examples and one linked vocabulary article; no defined sample, verified production histories, model comparison, or prevalence. | B1, B3, and C7 recognise the named categories. The exact `It’s not just X, it’s Y.` snippet triggered B3; the article body triggered C7 six times while B1 stayed below threshold at two terms in its worst paragraph and three total. H24/H25 cover register, date, and model drift. **Partly covered.** | The live checks can surface the forms, but this source cannot validate their severity or thresholds and warns that user-facing treatment may cause camouflage. | Record as public-tell and harm context only. Do not change B1/B3/C7 thresholds from this article; evaluate any anti-camouflage wording under C01/C05 separately. | pending | not started |
| C03: Gallagher says his oversensitive AI radar marked an academic article as AI-written before he noticed it was published in 2019. | Attributed anecdote linked to Gallagher's own post; direct evidence of one mistaken judgement, not a detector false-positive rate. | H3 and the `human-eyes/references/process.md` product boundary reject authorship verdicts; H10 requires source and date verification in journalism/academic contexts. **Fully covered as a caution, not as measured error evidence.** | No implementation gap supported by one anecdote; the linked post would need separate review for any broader claim. | Record only as a dated human-look-alike and source-date check; take no further product action from this anecdote. | pending | not started |
...
| C12: Gallagher uses perceived ChatGPT tics as prompts to inspect his reliance on lists and word choice; Clarke ultimately corrects the typo but becomes less bothered by future organic errors. | Gallagher material is attributed and linked; Clarke's conclusion is direct first-person evidence. Both reject a simple always-purge or always-preserve rule. | G3 distinguishes unnecessary listification from appropriate lists; the process requires context-specific changes and preservation of deliberate choices. **Fully covered.** | No gap supported beyond the optional C01/C05 anti-camouflage wording. | Record only as the craft-versus-camouflage boundary; take no further action apart from Mae's pending C01/C05 decision. | pending | not started |

## Recommendations

- C01: Pending Mae, test-adapt bounded anti-camouflage wording in report/rewrite guidance before any implementation.
- C02: Record as public-tell/harm context; do not change B1/B3/C7 thresholds from this article.
...
- C05: Pending Mae, include the source in a register-matched C7 evaluation and test anti-camouflage wording; make no immediate severity or threshold change.
...
- **C09:** Closed 2026-07-26 via DR-28. Yakura et al. is ingested as `yakura-llm-influence-spoken-communication.md`; cite it directly. Its word list rests on a written-prose measurement, and the words shipped to B1 through DR-165 and DR-165A.
...
- C05: not applicable - rejected 2026-07-17; C7 remains fail-on-any as a deliberate stance and no product change was made.
...
- C09: passed - the direct card `yakura-llm-influence-spoken-communication.md` exists and passes `python3 .agents/skills/source-ingest/scripts/validate_source.py`; this row now cites it instead of the indirect summary. Documentation only; no checker change to verify.
...
- **Findings resolved:** 2 - recorded the conflicting Permutive 2025-08-19 publication timestamp alongside the displayed/NewsArticle 2025-08-20 date, and corrected C05 to state that live C7 requires a fix at Balanced and All because it is a `strong_warning`. Focused re-check found 0 residual findings.
