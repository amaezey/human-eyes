...
- **Unsupported use:** G2 generic metaphors, H6 faux specificity, H7 neutrality collapse, a detector rule, a severity level, an authorship verdict, a model mechanism, a measured bias rate, or a claim that all consistent automation is exclusionary.
...
- The inherited G2 generic-metaphor, H6 faux-specificity, and H7 neutrality-collapse mappings were already removed in `pattern-opportunities.md`; C13 confirms that retirement from the complete article.
- The legacy H8 and H9 associations are retired as conceptual adjacency rather than source support.
- All recommendations except C04 remain `pending` / `not started`. C04's ruling makes no product change.
- C04 rejected 2026-07-18 via DR-121: no new `cannot determine` report outcome was added. The closed-source boundary, factual-fidelity limitation, and `Remaining findings` reporting already state what the prose workflow can and cannot establish; service-agent escalation remains outside product scope.
...
- C13: Maintain the G2/H6/H7 non-promotion and retire H8/H9 source-support labels.
...
- C04: not applicable - rejected 2026-07-18 via DR-121; existing closed-source and Remaining findings behavior stands, with no new report outcome or escalation feature.
