...
- **Patterns left on the table:** The exact flat-rhythm pair exposes an eligibility boundary in G9; the exact `This creates` example exposes candidate-versus-threshold behavior in H4; the useful `In summary` paragraph and explicit em-dash defense challenge fail-on-occurrence behavior; and the writing loop supplies process evidence rather than a new prose tell.
...
- Should G9's documented eligibility wording be reconciled with the implementation before Shankar's short three-sentence pair is used as a test?
...
- C18, C19, C21 approved 2026-07-26 via DR-27: Mae queued this work for later rather than ruling on its shape now. No checker, registry, or test change has been made and implementation has not started.
...
- 2026-07-17: Mae approved removing the E6 soft-scaffolding citation from the root README; the essay does not support that exact mapping. Pattern E6 unchanged.
- 2026-07-26: C03 rejected via DR-25. G3 `no-excessive-lists` is unchanged, so it keeps failing on either 8 or more list items across 2 or more blocks, or list items making up 30% or more of a document's lines. The line-ratio behaviour was measured across every sample corpus before the ruling. Only four documents clear 30%, one of which also trips the item-count branch. The ratio branch's sole independent effect is flagging a 328-word human blog post carrying a single three-item list, alongside two sub-100-word fixtures; every corpus detection comes from the item-count branch. A minimum of 8 items on the ratio branch would have cleared all three while preserving both existing test fixtures and changing no corpus flag, and Mae declined it.
- 2026-07-26: C22 rejected via DR-153. Shankar prescribes the Somebody-Wanted-But-So-Then frame as good craft and uses it as a rewrite instruction he gives a model; he reports no unprompted machine production of it and supplies no frequency claim. Before the ruling the shape was measured with a matcher verified against his own GPT-4 summarisation example, requiring a wanting verb plus `but`, then a `So` opener, then a `Then` opener within a short window. It returns zero hits across the generated corpus and one across the human corpus, and that one is a writing-advice article recommending the technique rather than a decision narrative. His example also passes every check in the live registry. The project corpora contain no technical decision narrative, engineering writeup, or design doc, so the null holds for the writing the project has and does not speak to the register this claim names. Mae declined to gather that corpus and ruled to add nothing.
...
- C04 approved 2026-07-17: reconcile G9's documented eligibility with the implementation. Implemented in commit 13e235f; G9 now skips prose under 100 words or under 6 sentences per the documented eligibility.
...
|---|---|---|---|---|---|---|
| C01: While writing and reviewing technical papers and blog posts, Shankar reports encountering LLM-generated writing that feels off; she also values LLMs for early drafts, summaries, and rephrasing, and frames much current reading as machine-generated. | Direct first-person practitioner framing; unspecified tools, prompts, counts, and outcomes; the statement that much reading is now machine-generated is unquantified author framing, not a prevalence result. | Partly covered by the source-tier rules and product boundary, which allow craft guidance while prohibiting authorship inference. | The current general evidence note calls this one of the strongest craft references and says the claims directly support several checks without restating the bounded anecdotal method, broad machine-generated-reading frame, or workflow substance. | Record the bounded method, unquantified framing, workflow scope, and no-authorship limit in source mappings; no checker change. Verify by source-card review. Rejected 2026-07-26 (DR-140): Mae left the record-layer wording unchanged. | rejected | not applicable |
| C02: Empty paragraph-final summary sentences can sound conclusive while adding nothing, and Shankar has no reliable recipe for a substantive parting thought. | Direct practitioner observation plus two selected examples; the failed-recipe statement is a null process result, not a model evaluation. | Partly covered by H2, agent assessment `semantic_redundancy`, and process guidance to remove paragraph recaps. Focused H2 execution on the two examples returned `Tidy paragraph endings: 0`. | The exact examples are outside H2's lexical and structural candidate shapes, and no outcome evidence justifies a new threshold. | Add the examples only to H21 or a future semantic-redundancy evaluation with human controls before any rule change. | approved | implemented |
| C03: Bullets and outlines are overused when connected ideas need context; lists remain useful for parallel, independent items. | Direct practitioner observation with an explicit legitimate-use condition; no count or rate. The two-item list used in focused execution belongs to C02 and is a reviewer-selected short-list control, not an author-labelled legitimate-list example. | Partly covered by G3 and process guidance to preserve lists that compare real content. Focused execution on that isolated two-item control returned a finding from a 50% line ratio. | G3's ratio can fail a short two-item list in isolation and cannot judge whether items are independent or context-dependent. | Evaluate G3's line-ratio behavior on short legitimate lists and prose-conversion cases before adapting thresholds or guidance. | rejected | not applicable |
| C04: Flat sentence lengths reduce rhythm, emphasis, attention guidance, and pace; varied lengths can improve the same technical explanation. | Direct craft claim with one author-labelled bad/good pair; no reader test, authorship labels, or quantitative rhythm metric. | Partly covered by G9 and process guidance. Focused G9 execution skipped both examples: bad was 79 words and 3 sentences; good was 54 words and 3 sentences. Both examples separately triggered B4 triad candidates, which is unrelated to the claimed rhythm contrast. | The exact evidence pair does not exercise G9, and the catalogue says 100-plus words while the implementation skips only when both sentence count is below 6 and words are below 100. | Reconcile G9's documented eligibility with code, then evaluate the pair only as a short-form control; do not infer a threshold from it. | approved | implemented |
| C05: A sentence can obscure its topic by choosing the wrong grammatical subject; the source contrasts `Readers are better guided...` with `Choosing the right subject...`. | Direct grammatical explanation and selected rewrite pair; no measured sample or reader outcome. | Not covered. H21 tracks the issue, while H4 and `referential_clarity` handle vague referents rather than general subject-topic alignment. | No live assessment asks whether the grammatical subject is the sentence's actual topic. | Test H21 on matched examples and decide whether subject-topic alignment belongs in agent assessment or rewrite-only guidance. | pending | not started |
...
| C07: Vague prose refers to undefined ideas and makes unsupported claims, illustrated by unnamed experts and an unspecified productivity impact. | Direct practitioner observation and selected example; no frequency or model identity. | Fully covered across A5 `no-vague-attributions` and the general `underspecified_language` assessment. Focused A5 execution matched `experts say`. | Deterministic A5 covers only the unnamed-authority portion; the complete Audit is needed for missing criteria and stakes. H10's default branch adds no coverage here. | Retain the A5 and `underspecified_language` mappings and add no new rule; use complete-Audit evidence when evaluating the broader example. | pending | not started |
| C08: Demonstratives such as this, that, these, and those become unclear without a same-sentence or immediately prior noun; Shankar says humans make this mistake too. | Direct observation, explicit human look-alike, and one `This creates friction in production` example. | Fully covered by `referential_clarity` in a complete Audit and partly by H4/H5. Focused H4 recognized `this creates` as one candidate but stayed below its three-candidate threshold. | The deterministic finding threshold does not make the candidate invisible, but a surface-only report does not decide antecedent clarity; human occurrence defeats authorship use. | Retain the complete-Audit mapping and human look-alike note; do not lower a threshold from one example. | pending | not started |
| C09: Fluent technical prose can fail when it assumes audience knowledge and explains neither a term nor the hard part; a human must fill the gap. | Direct practitioner observation and selected attention-mechanism example; no audience study or comprehension measurement. | **not covered:** this claim is about explaining too *little*, and `audience_knowledge_mismatch` (D6) asks about explaining too *much*. The 2026-07-17 note that C09 produced that record was wrong on direction; SEO Engine C22 is the record's source of record from 2026-07-27 (DR-169). Partly covered by H21 and process meaning preservation. | H10's default branch has no watchlist, its academic/student branches do not assess reader prior knowledge, and no current check asks what the intended reader already knows. | Rejected 2026-07-27 (DR-173): outside what human-eyes covers. Rule 1 limits the product to habits AI overuses, and under-explaining to a specialist reader is what an expert human does writing for themselves; a model with no picture of its reader errs the other way, which D6 covers. No detector in either lane. | rejected | not applicable |
| C10: LLMs may produce dubious technical terminology; Shankar gives `retrieval grounding` and says it is not a term she has heard. | Direct anecdotal observation and an explicitly hedged personal-knowledge boundary; the source does not prove the term nonexistent or name the generating model/version. | Partly covered by H10 source and factual verification in named genres and by the closed-factual-record process. | General technical prose in the default branch has no terminology-verification assessment, and the example cannot support a lexical ban. | Record as a terminology-verification candidate only; require domain sources before treating any term as invented. | rejected | not applicable |
...
| C12: Intentional repetition and some predictability can clarify or reinforce complex material when the repetition is purposeful. | Direct craft judgment and one vector-database paraphrase example; no reader study. | Fully covered by process guidance to preserve deliberate devices, H14's deliberate-anaphora tolerance, and `semantic_redundancy` for repetition that adds nothing. | The source's example is paraphrastic repetition, not necessarily the same construction as H14. | Record as a deliberate-use control; take no further product action without evaluation. | pending | not started |
| C13: Signposts such as `essentially`, `in short`, and `the point is` can help readers reorient if useful content follows. | Direct craft judgment and one concrete author-labelled `Essentially` example; no reader test. Treating the separate `In summary` paragraph as content-bearing is the reviewer's application of Shankar's criterion. | Challenges current behavior. The concrete `Essentially` example passes G8 and E6, while the reviewer-selected content-bearing paragraph beginning `In summary` fails G8 on one occurrence. | G8 remains occurrence-based for listed conclusion signposts. | Take no product action; leave G8 unchanged. | rejected | not applicable |
| C14: Parallel structure can organize related ideas when each clause contributes new information. | Direct craft judgment and one three-clause example; no reader study. | Fully covered by process guidance to preserve parallel forms that compare real content and by B4/`formulaic_parallelism` only when symmetry substitutes for information. The exact source example did not trigger B4 in focused execution. | No material gap for this bounded legitimate-use claim. | Keep as a control for B4 and take no further action. | pending | not started |
| C15: Predictable headings such as `Why X fails`, `What to do instead`, and `How to know if it worked` can be clear when their sections deliver. | Direct craft judgment with a three-heading schema; no outcome study. | Fully covered. G6 targets an identical short label repeated three times, not distinct echoing headings, and the three source headings passed focused G6/G8 execution when paired with distinct body text. | G6's implementation can also count repeated short body lines, but that is outside this source claim. | Keep the heading schema as a legitimate control; take no further source-driven action. | pending | not started |
...
- C02: Evaluate the exact empty-summary examples with human controls under H21 and `semantic_redundancy` before adapting H2.
- C03: Test G3's line-ratio behavior on short legitimate lists and context-dependent prose before any threshold or guidance change.
- C04: Reconcile G9's documented and implemented eligibility, then use the pair only as a short-form control.
...
- C07: Retain A5 and complete-Audit mappings; add no new rule.
- C08: Retain `referential_clarity` and H3 candidate mappings with the explicit human look-alike; do not infer a lower threshold.
...
- C13: Leave G8 unchanged; add no content-bearing gate.
- C14: Keep the source example as a legitimate parallelism control and take no further action.
- C15: Keep the heading schema as a legitimate G6/G8 control and take no further action.
...
- C04: passed - G9 eligibility reconciled with documentation in commit 13e235f (skip under 100 words or under 6 sentences; `human-eyes/scripts/grade.py` plus regression tests in `dev/evals/tests/test_grade.py`); `python3 -m unittest dev.evals.tests.test_grade` passes on 2026-07-17.
- C17: not applicable - rejected 2026-07-17; C7 remains fail-on-any as a deliberate stance and no product change was made.
...
- C09: withdrawn 2026-07-27 (DR-169) - the `audience_knowledge_mismatch` record recorded here on 2026-07-17 asks about explaining what the audience already knows, the opposite of what C09 describes, and it shipped with no register row. The record stays, sourced to SEO Engine C22; C09 itself is re-queued unruled as DR-173.
- C10: not applicable - rejected 2026-07-18 via DR-121; no external-source terminology assessment or lexical ban was added.
- C13: not applicable - rejected 2026-07-19 via DR-134 option B; G8 remains unchanged, and the content-bearing control still fails.
- C03: not applicable - rejected 2026-07-26 via DR-25; G3 `no-excessive-lists` keeps both its 8-items-across-2-blocks branch and its 30% line-ratio branch, and no minimum item count or document length was added.
- C22: not applicable - rejected 2026-07-26 via DR-153; no SWBST matcher was built, `formulaic_parallelism` was not extended, and no guidance was added.
- All other rows (C01, C05-C08, C11-C12, C14-C16, C18-C21, C23): not applicable - recommendations remain pending; no product, checker, registry, test, hypothesis, or guidance change was authorized or made.
...
- **Findings resolved:** Six material findings resolved: extraction provenance now records reuse of the prior Markdown body after current-HTML comparison; C01 restores the source's LLM-generated wording and unquantified machine-generated-reading frame; C03/C13 label reviewer-selected controls; C01 is partly covered; C07/C09 remove unsupported H10/H8 coverage; and the standalone C01 recommendation matches its authoritative table row. Final reviewer verdict: `VERDICT: focused re-check passed with no unresolved findings`.
