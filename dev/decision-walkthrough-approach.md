...
4. **One decision file:** `dev/decision-register.md`. Numbered rows DR-01 upward (plus POS-01... for the POS-dependent list). Nothing pending lives anywhere else. Never invent new queues, lanes, groups, or orderings. Decision statuses are exactly three — `pending`, `approved <date>`, `rejected <date>` — use them verbatim and never coin new ones ("record-only", "parked", etc.) in rows or in chat; parked-for-evaluation was removed by Mae 2026-07-17. An approved evaluation or deferred design stays `pending` until its concrete result is ruled on.
...
Decided and fully closed through question 20: rows DR-01..11, DR-39/40, DR-43, DR-106 (with DR-30), DR-111 (journalism watchlist), DR-112 (poetry watchlist), DR-113 (#19 residue families), DR-114 (components 1-2 implemented, 3-4 declined), DR-115 (with DR-44: components 4 and 6 built as #58 `no-paragraph-anaphora` and #59 `no-heading-one-liners`, commit e1002ca; components 1, 2, 3, 5 declined). Numbering ruling from DR-115: new patterns take the next plain number (58, 59, ...); never add new letter-suffixed variants — the five existing ones (10a, 23a, 31a, 35a, 35b) are grandfathered. Orphan rule (Mae, 2026-07-17): no claim may end up pending on a card while every register row citing it is closed. When closing a row, sweep every claim its Extends target cites; any claim left unruled goes back in the queue as its own row and gets asked. Never mark a claim decided with a ruling Mae did not give, and never leave it stranded either — ask. DR-116 decided (option a): run the #31a threshold calibration; corpus measured (emoji near-absent in essays), genre-control fixtures not yet built; row unstamped until the calibration record lands. Orphan sweep 2026-07-17: 12 claims were stranded pending on cards while every register row citing them was closed — several because cards were renumbered after refresh, so old evidence keys point at different claims than the ones rulings were recorded on. All 12 re-queued as DR-141..152 (Extends left blank deliberately; provenance is in each row's text). Register is now 152 rows. Next open rows: DR-141..152, then back to DR-116 implementation. Retirements DR-12/13/29 need verification against their cards before being asked. The POS-01/POS-02 list waits on Mae's tagger from another project.
