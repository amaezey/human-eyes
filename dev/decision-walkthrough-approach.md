...
The register was normalised 2026-07-18 (commit 8e99aab): one seven-column layout, statuses clean, link-closures visible, truncations repaired, "How to read this file" header canonical. Current counts after DR-15: 97 open, 55 decided, 3 pos-dependent-pattern (POS-01, POS-02, DR-144 wait on Mae's tagger).

Next by current product significance: DR-16, then the remaining executable variant groups DR-17..20 and DR-93. Re-scan the full pending register after each closure rather than treating this as a fixed row-order queue. Agent-judgement branches and product-unlocking evaluations follow the executable programmatic gaps. Documentation, provenance, caveats, citation mappings, and source-record-only rows are last. A like-for-like social/chat corpus for any future emoji rerun is noted in the DR-116 record but not queued.
