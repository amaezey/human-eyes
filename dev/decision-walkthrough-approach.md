...
4. **One decision file:** `dev/decision-register.md`. Numbered rows DR-01 upward (plus POS-01... for the POS-dependent list). Nothing pending lives anywhere else. Never invent new queues, lanes, groups, or orderings. Decision statuses are exactly four — `pending`, `approved <date>`, `rejected <date>`, `parked-for-evaluation <date>` — use them verbatim and never coin new ones ("record-only" etc.) in rows or in chat (Mae, 2026-07-17).
