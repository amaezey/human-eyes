...
|---|---|---|---|---|---|---|
| C01: Llama 3.2-3B Instruct differs from Base in lemma-POS frequency, with the largest reported increases headed by `nuanced_ADJ` +8342.8%, `nuance_VERB` +6301.7%, `firstly_ADV` +4794%, `reliance_NOUN` +3193.6%, and `generalizability_NOUN` +3124%. | Direct Base-versus-Instruct comparison of about 2.3m and 2.2m continuation words from 9,853 PubMed prompts; one model pair, one register, and uncorrected chi-square screening. | partly covered: #7 includes `nuanced` and some related forms; H24 proposes register-specific vocabulary density. Focused execution passed `nuance reliance generalizability` with zero #7 matches. | The live check lacks the complete source profile, lemma-POS distinctions, effect sizes, Base comparison, and multiple-testing control. | Test-adapt only as a source-specific H24 evaluation profile after independent statistical recomputation and matched-register controls; do not change product files now. | pending | not started |
