...
- C13 rejected 2026-07-19 via DR-134 option B: #44 remains occurrence-based for its listed conclusion signposts. No content-bearing gate was added.
...
| C13: Signposts such as `essentially`, `in short`, and `the point is` can help readers reorient if useful content follows. | Direct craft judgment and one concrete author-labelled `Essentially` example; no reader test. Treating the separate `In summary` paragraph as content-bearing is the reviewer's application of Shankar's criterion. | Challenges current behavior. The concrete `Essentially` example passes #44 and #47, while the reviewer-selected content-bearing paragraph beginning `In summary` fails #44 on one occurrence. | #44 remains occurrence-based for listed conclusion signposts. | Take no product action; leave #44 unchanged. | rejected | not applicable |
...
- C13: Leave #44 unchanged; add no content-bearing gate.
...
- C13: not applicable - rejected 2026-07-19 via DR-134 option B; #44 remains unchanged, and the content-bearing control still fails.
- C03: not applicable - rejected 2026-07-26 via DR-25; #31 `no-excessive-lists` keeps both its 8-items-across-2-blocks branch and its 30% line-ratio branch, and no minimum item count or document length was added.
