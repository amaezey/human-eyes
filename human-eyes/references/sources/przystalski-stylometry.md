...
| C14: In the GPT-4/Wikipedia comparison, GPT-4 uses `significant`, `notable`, and `despite` more heavily. | Direct SHAP/example interpretation for one model, prompt family, and domain; no bare-word effect size, specificity, or universal threshold. | #7 exact implementation does not include these three bare words; its cluster list and threshold are different. Coverage is not covered for the exact claim. | A universal word addition would ignore topic, model, prompt, frequency, and human controls. | Do not add bare words; if evaluated, use model/date/genre-matched densities and human controls. | pending | not started |
...
- C01-C25: not applicable - no product change approved or implemented during source review.
