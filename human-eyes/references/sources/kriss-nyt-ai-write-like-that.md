...
- `no-rhetorical-questions` (#29); partial coverage because one immediate question-answer candidate does not meet the live two-candidate threshold.
...
- C11 approved 2026-07-17: fix #26 double-counting and the #27 runtime/documentation divergence. Implemented in commits 13e235f (#26 token boundaries) and 7543052 (#27 token counting plus documented `hum`, `humming`, `soft`, and `settle` added). All other rows remain pending.
...
| C07: Kriss observed recurring fiction names Elara Voss/Elena Voss and Kael and reported hundreds of post-2023 Amazon examples. | Direct observation plus an unreported Amazon search method; no denominator, genre control, date capture, or reproducible count. | `paech-antislop-sampler.md` separately reports corpus-level Elara/Kael concentration; no live name rule; **not covered**, appropriately. | A name blacklist would create severe false positives and the essay alone does not validate a rate. | Keep as source-specific fiction context; do not add a name rule. | pending | not started |
...
| C13: Chat responses can interrupt themselves with a short question-answer beat such as `And honestly? That's amazing.` | Direct example and author observation, said to be more common in conversation than essays; no frequency or model version. | Focused probe finds one candidate but `no-rhetorical-questions` passes because its threshold is two; **partly covered**. | The exact source example does not produce a finding, and the one-versus-two threshold lacks evidence from this article. | Add the example only to a future matched conversational evaluation; do not change the threshold from this source alone. | pending | not started |
| C14: Generated prose can overuse tricolons, including three in a little over 100 words and the literary variant `No X. No Y. Just Z.` | Direct count over an unattributed viral excerpt plus a quoted Bing example; unknown original provenance and no comparison corpus. | Focused exact-excerpt probes: the viral passage's three sentence-level tricolons produce zero #10 candidates, while #9 and #25 surface `No family. No calls. Just silence.` The separate Bing quotation produces three #10 coordinated-list candidates. #10a passes both excerpts because neither reaches four candidates in 300+ words. Coverage is **partly covered and challenged**. | #10 does not recognize the construction Kriss counts, and the source's short-text density example is not equivalent to #10a; quotation, comedy, and deliberate human rhetoric require controls. | Evaluate #10 recognition of sentence-level tricolons and #10a's length/density design against matched genres before any product change; preserve quotation/provenance labels. | pending | not started |
| C15: Lightly dismissive model prose repeatedly uses `an X with Y and Z`, with examples from ChatGPT, Grok, and Claude. | Three direct prompted examples across named products; builds, dates, settings, raw transcripts, and repetitions are absent. | Focused probe `a Reddit troll with Wi-Fi and billions` triggers none of #7, #9, #10, #10a, #25, or #42; **not covered**. | The construction may be a comedy/roast convention and needs human controls; this essay supplies no rate. | Add a pending pattern-opportunity row for matched, genre-aware evaluation; make no checker change. | pending | not started |
...
- C13: Use the one-question example only in a future matched conversational evaluation.
- C14: Evaluate #10 recognition of sentence-level tricolons and #10a's length/density design with matched length and genre controls.
