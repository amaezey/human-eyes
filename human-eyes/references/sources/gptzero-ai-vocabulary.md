...
- C07 approved 2026-07-17: fix live #7 nested-match overcounting and repeated-phrase undercounting. Implemented in commit 13e235f. All other rows remained pending at that point.
- C06 approved 2026-07-17: closed by the same #7 counting fix as C07 (commit 13e235f); the prior decision batch recorded C07 but left C06 pending. The six overcounted source examples now pass.
...
- C02 and C08 approved, and C09 and C16 rejected, 2026-07-18 via DR-127: #7 now identifies GPTZero's 100-row AI Vocabulary client payload without a false revision month; mismatch and data-quality details stay in this source record, with no detector change or added product caveat.
...
- C09: Add no human-use or probability caveat to the product; #7 continues to surface the programmatic matches.
...
- C02: passed - #7 retains the exact ordered 100-item payload and the catalogue identifies it as GPTZero's 100-row AI Vocabulary client payload.
...
- C06: passed - the #7 counting fix in commit 13e235f resolved the nested-match overcounting; rerunning `check_ai_vocabulary` on the six source example sentences (rows 13, 42, 52, 64, 66, and 76) on 2026-07-17 returned `passed=True` for all six.
- C07: passed - the #7 counting fix landed in commit 13e235f (`human-eyes/scripts/grade.py` plus regression tests in `dev/evals/tests/test_grade.py`), correcting nested-match overcounting and repeated-phrase undercounting; `python3 -m unittest dev.evals.tests.test_grade` passes on 2026-07-17.
- C08: passed - the false `April 2026` revision wording was removed from `grade.py`, `patterns.json`, and the generated `patterns.md`.
- C09: not applicable - rejected via DR-127; no human-use or probability caveat was added and #7 behavior is unchanged.
