...
- C02: not applicable - pending source-record recommendation; focused live checks of all six exact source phrases found zero `no-excessive-hedging` findings and two `no-filler-phrases` findings corresponding to `it is important to note` and `it is worth mentioning` (reported spans `it is important to note`, `worth mentioning`); no product change requested.
