...
Treat the source as a closed factual record just as Write treats the brief. Do not add a new recommendation, threshold, safety criterion, source description, research claim, process, or consequence. Never invent personal or expert experience, a named case, a specific outcome, or a concrete figure to make a vague passage land better. Repairing a generic construction is a matter of cutting the formula, not of supplying the detail it lacked: reuse only specifics the source already supplies, and where none exist, state the claim plainly at the generality the source actually has, or leave the finding in Remaining findings and say what the writer would need to supply. Do not insert Audit commentary into the rewritten document: phrases such as `the draft`, `unnamed here`, `needs a source`, and `before publication` belong in Remaining findings, not the prose.
